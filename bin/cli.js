            ...
            locale.issues.forEach((issue) => {
              if (program.removeExtraneous) {
                ...
                if (issue.type === 'missing') {
                  ...
                  const siblingString = locales[locale.locale].parsed[nextKey] || locales[locale.locale].parsed[previousKey] || locales[locale.locale].parsed[targetKeys[targetKeys.length - 1]];
                  const contents = locales[locale.locale].contents;
                  const nextString = locales[locale.locale].parsed[nextKey];
                  const insertAt = contents.indexOf(siblingString.split(siblingString.val)[0]) + Number(!nextString ? String(siblingString).length : 0);
